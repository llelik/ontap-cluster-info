...
class CustomEventHandler(Script):
    class Meta:
        name = "Custom ONTAP SVM Event Handler"
        description = "Script to handle custom ONTAP SVM events in NetBox"
    ...
    def run(self, data, commit):
        ...
        job_template_id = '8'
        self.log_info('Tring to load data into extra_vars...')
        extra_vars = {
            'payload_data': {
                'name':             data.get('name', ''),
                'cluster':          (data.get('cluster') or {}).get('name', ''),
                'aggregate':        (data.get('aggregate') or {}).get('name', ''),
                'aggr_list':        [aggr.get('name', '') for aggr in data.get('aggr_list', [])],
                'svm_type':         data.get('svm_type', ''),
                'subtype':          data.get('subtype', ''),
                'admin_state':      data.get('admin_state', ''),
                'language':         data.get('language', ''),
                'ipspace':          data.get('ipspace', ''),
                'root_volume_name': data.get('root_volume_name', ''),
                'site':             (data.get('site') or {}).get('name', ''),
                'tenant':           (data.get('tenant') or {}).get('name', ''),
                'comments':         data.get('comments', ''),
                'svm_comment':      data.get('svm_comment', ''),
        ...
        if response.status_code == 201:
            ...
            return f"Failed to launch AWX job. Status code: {response.status_code}, Response: {response.text}"
